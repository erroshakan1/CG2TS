...
def get_radius_and_defect_type(atom_name, cos_alpha):
    ...
    if atom_name in ['NC3', 'NH3', 'PO4']: 
        atom_radius = 3.4796
    elif atom_name in ['GL1', 'GL2', 'GLA', 'NAB']:
        atom_radius = 2.413
    else:
        atom_radius = 2.638
...
def get_selection_phaseseparated(gro, cutoff_l, cutoff2):
    ...
    if len(gla) > 0:
        ...
        upper = gro.select_atoms(f'(name ROH or name GL2) and resid {" ".join(map(str, up_resids))}')
        lower = gro.select_atoms(f'(name ROH or name GL2) and resid {" ".join(map(str, lo_resids))}')
...
def main(args):
    ...
    seltail = {'DPPC': 'name C4*', 'DPPG': 'name C4*', 'CHOL': 'name C*', 'DTAP2': 'name C4*'}
    ...
    for ts in gro.trajectory[args.start_frame:args.end_frame:args.skip]:
        ...
        for leaflet_name, leaflet_ag, is_upper in [('Upper', upper, True), ('Lower', lower, False)]:
            ...
            mesh_r3.paint_uniform_color([0.9, 0.9, 0.9]) # Light Gray (Original notebook color)
            ...
            # Enable back-face rendering to match original notebook behavior
            o3d.visualization.draw_geometries([mesh_r1, mesh_r2, mesh_r3], window_name=f"{leaflet_name} Leaflet Curvature Regions", mesh_show_wireframe=True, mesh_show_back_face=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--leaflet-cutoff', type=float, default=15.0, help='Cutoff for LeafletFinder.')
